...
Title = "Viper Cat Hub BloxFruit [Goddess]",
...
Content = "Stopped Event Island Spawn",
...
local AutoAttackSegments = v16.Levi:AddToggle("ToggleAutoAttackSegments", {
    Title = "Auto Attack Leviathan Segments (Updating)",
    Description = "Don't Use Updating",
    Default = false
})

AutoAttackSegments:OnChanged(function(state) 
    if state then 
        spawn(function() 
            while AutoAttackSegments.Value do 
                local seaBeasts = game:GetService("Workspace").SeaBeasts
                
                for _, segment in ipairs(seaBeasts:GetChildren()) do
                    if segment:IsA("Model") and segment.Name:find("Leviathan") and segment:GetAttribute("HealthEnabled") then
                        if segment:FindFirstChild("PrimaryPart") then -- Ensure segment has a primary part
                            Tween(segment.PrimaryPart.CFrame * CFrame.new(0, 450, 0))
                            ...
                            for _, tool in pairs(backpack:GetChildren()) do
                                if tool:IsA("Tool") then
                                    if tool.ToolTip == "Melee" or tool.ToolTip == "Blox Fruit" or tool.ToolTip == "Sword" or tool.ToolTip == "Gun" then
                                        ...
                                        -- F Skill
                                        virtualInput:SendKeyEvent(true, 102, false, player.Character.HumanoidRootPart)
                                        virtualInput:SendKeyEvent(false, 102, false, player.Character.HumanoidRootPart)
                                        wait(0.2)
                                    end
                                end
                            end
...
v16.Draco:AddButton({
    ...
    Callback = function()
     loadstring(game:HttpGet("https://raw.githubusercontent.com/viperobii/viperbloxme/refs/heads/main/cokka"))()
    end
})
...
Title = "Viper Hub Goddess",

Content = "Viper Hub Loaded",
